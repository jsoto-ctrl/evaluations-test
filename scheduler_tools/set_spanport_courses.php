...
## This script is the first of the series. It populates scheduler.sections after gathering all info from the IUIE roster file (saved as csv, Unif LF).
## Script 2 is set_spanport_users.php
...
$table = "sections";
	...
	list($section_string, $course_string, $inst_string,$time_string,$days_string,$loc_string)=explode(",",$line);
		...
		#ACP classes are still reported by IUIE, but location and days are missing. Test against that. 
				if ($course_number > 1 && $course_number < 800 && $loc != "ion:" && $days != "s:") {
				...
				echo "Time exploded is $start / $end but hour is $hour<br />";
					if ($process) {
					echo "<font color=navy>Insertion here</font><br />";
					
				$sql = "INSERT INTO $table (section,course,last,first,days,time,start,end,room,active)
				VALUES
				(\"$section\",\"$course\",\"$inst\",\"$inst2\",\"$days\",\"$hour\",\"$start\",\"$end\",\"$loc\",\"1\")";
